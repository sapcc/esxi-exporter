...
class EsxiOnlineStateCollector(BaseCollector):
    ...
    def collect(self):
        ...
        for host in vc_hosts:
            vc_state = host.overallStatus == 'green'
            gauge_metric.add_metric(labels=[getenv('vcenter_url'), host.name, 'vcenter'], value=vc_state)

            nb_state = self.netbox.is_host_active(host)
